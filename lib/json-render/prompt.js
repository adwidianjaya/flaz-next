...
const handlePromptRoute = async (request) => {
  ...
  const providerKey = configs.DEFAULT_LLM_PROVIDER || "google";
  const modelName = configs.DEFAULT_MODEL_NAME || "gemini-1.5-flash-latest";
  const temperature = parseFloat(configs.AI_TEMPERATURE) || 0.7;
  const systemPromptPrefix = configs.SYSTEM_PROMPT_PREFIX || "";

  const providers = { xai, openai, anthropic, google, openrouter };
  const provider = providers[providerKey] || google;
...
const buildSystemPrompt = ({
  ...
  systemPromptLines.push(
    JSON.stringify({
      op: "replace",
      path: "$elements.greeting-text.props.className",
