...
// subscribe(store, () => {
//   console.log(
//     "store has changed to",
//     store.definition?.elements?.["form-title"]?.props,
//   );
// });
  ...
  return [
    snap.schema,
    {
      setSchema: (schema) => {
        ...
        // console.log("...store.elementIndex", store.elementIndex);
      },
    },
  ];
};
  ...
  return [
    snap.selectedElement,
    {
      ...
      updateSelectedElementProp: (key, value) => {
        if (!store.selectedElement?.props) {
          return;
        }
        ...
        if (store.schema.elements) {
          ...
          if (schemaElement && schemaElement.props) {
            store.elementIndex[store.selectedElement.elementId] = schemaElement;
            set(schemaElement.props, key, value);
          }
        }

        if (store.definition.elements[store.selectedElement.elementId]) {
          set(
            store.definition.elements[store.selectedElement.elementId].props,
            key,
            value,
          );
        }
