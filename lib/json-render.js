...
const deployPromptRoute = async (request) => {
  ...
  systemPromptLines.push(
    JSON.stringify({
      ...
      value: {
        ...
        props: {
          text: {
            $format: ["Hello, ", "/states/form/email", "!"],
          },
  ...
  if (context) {
    systemPromptLines.push(
      "\nCurrent page definition context (use as reference for update operations):",
    );
    systemPromptLines.push(JSON.stringify(context));
  }
...
/*
  $state: "/states/path"
  $state: ["/states/path", "default"]
  $format: "/states/path"
  $format: ["Hello: ", "/states/path", ...]
  $format: ["Hello: ", { $state: "/states/path" }, ...]
  $format: ["Hello: ", { $state: ["/states/path", "default"] }, ...]
  $format: ["Hello: ", { $uppercase: "/states/path" }, ...]
  $set: ["/states/path", "value"]
  $set: ["/states/path", { $add: [ "/states/path", 1 ] } ]
  $set: ["/states/path", { $add: [ { $state: "/states/path" }, 1 ] } ]
  $unset: "/states/path"
  $batch: [
    { $set: ["/states/path", "value"] },
    { $set: ["/states/others", "value"] },
    ...
  ]
  $cond: [ { $eq: ["/states/path", "value"] }, "true", "false" ]
  $and: [ { $eq: ["/states/path", "value"] }, { $gt: ["/states/path", "value"] } ]
  $or: [ { $eq: ["/states/path", "value"] }, { $lt: ["/states/path", "value"] } ]
  $switch: [ 
    [ { $lte: ["/states/path", "value"] }, "true" ],
    [ { $lte: ["/states/path", "value"] }, "true" ],
    "default" 
  ]
  $set: ["/states/path", { $cond: [ { $eq: ["/states/path", "value"] }, "true", "false" ] } ]
  $cond: [ 
    { $eq: ["/states/path", "value"] }, 
    $batch: [
      { $set: ["/states/path", "value"] },
      { $set: ["/states/others", "value"] },
      ...
